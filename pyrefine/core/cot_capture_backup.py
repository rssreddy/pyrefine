"""
Change-of-Thought (CoT) Capture Module
...
class ChangeOfThoughtAnalyzer:
    ...
    def capture_cot(self, response_text: str, iteration: int) -> CoTCapture:
        ...
        logger.info(
            f"Captured CoT for iteration {iteration}: "
    ...
    def should_continue_based_on_cot(self) -> bool:
        ...
        if current_cot.overall_confidence < self.config['confidence_threshold']:
            logger.info(f"Continuing: Low confidence ({current_cot.overall_confidence:.2f})")
            return True

        # Continue if coherence is low
        if current_cot.reasoning_coherence < 0.7:
            logger.info(f"Continuing: Low coherence ({current_cot.reasoning_coherence:.2f})")
            return True

        # Continue if there are significant thought changes (indicates active refinement)
        if len(current_cot.thought_changes) > 0:
            logger.info(f"Continuing: Active thought changes detected")
            return True

        # Stop if we have consistent high-quality reasoning
        logger.info("Stopping: High confidence and coherence achieved")
        return False
    ...
    def reset(self):
        ...
        self.cot_history = []
        logger.info("CoT analyzer reset")
